...
func NewEncodingRequest(file string, segment int64) *EncodingRequest {
	return &EncodingRequest{file, segment, make(chan *[]byte), make(chan error)}
}

func NewWarmupEncodingRequest(file string, segment int64) *EncodingRequest {
	return &EncodingRequest{file, segment, nil, nil}
}
...
func (r *EncodingRequest) getCacheKey() string {
	h := sha1.New()
	h.Write([]byte(r.file))
	return fmt.Sprintf("%x.%v", h.Sum(nil), r.segment)
}
...
func NewEncoder(cacheDir string, workerCount int) *Encoder {
	...
	go func() {
		for {
			...
			data, err := execute(FFMPEGPath, EncodingArgs(r.file, r.segment))
			if err != nil {
				r.err <- err
			}
			r.sendData(&data)
...
func (e *Encoder) Encode(r EncodingRequest) {
	go func() {
		...
		e.reqChan <- r
		e.reqChan <- *NewWarmupEncodingRequest(r.file, r.segment+1)
		e.reqChan <- *NewWarmupEncodingRequest(r.file, r.segment+2)
	}()
}

func EncodingArgs(videoFile string, segment int64) []string {
