...
func (s *StreamHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// -acodec = audio codec
	// --timelimit
	if err := s.cmdHandler.ServeCommand(FFMPEGPath, []string{"-timelimit", "30", "-ss", fmt.Sprintf("%v.00", startTime), "-t", fmt.Sprintf("%v.00", hlsSegmentLength), "-i", filePath, "-strict", "-2", "-vcodec", "libx264", "-acodec", "aac", "-pix_fmt", "yuv420p", "-r", "25", "-f", "mpegts", "-force_key_frames", "00:00:00.00", "-x264opts", "keyint=25:min-keyint=25:scenecut=-1", "-"}, w); err != nil {
